...
class TestStageTransitionPolicyContract:
    ...
    def test_formation_to_breathing_stabilization(self):
        ...
        assert next_stage == Stage.FINAL_BREATHING or policy.state.current_stage == Stage.FINAL_BREATHING
    ...
    def test_breathing_to_pre_start_loop_mode(self):
        ...
        if hasattr(settings, 'loop_mode') and settings.loop_mode:
            assert next_stage == Stage.PRE_START or policy.state.current_stage == Stage.PRE_START
        else:
    ...
    def test_fallback_timeout_enforcement(self):
        ...
        assert next_stage is not None, "Stage should timeout and transition after extreme duration"
