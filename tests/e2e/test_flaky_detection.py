...
class TestFlakyDetection:
    ...
    def test_detect_flaky_unit_tests(self):
        ...
        flaky_tests = self._run_multiple_test_runs(
            test_paths=["tests/unit/"], num_runs=5, test_type="unit"
        )
        ...
        if flaky_tests:
            flaky_list = [
                f"{test['name']}: {test['pass_count']}/{test['total_runs']} passes"
                for test in flaky_tests
            ]
            pytest.fail(
                "Flaky unit tests detected:\n"
                + "\n".join(flaky_list)
    ...
    def test_detect_flaky_integration_tests(self):
        ...
        """
        flaky_tests = self._run_multiple_test_runs(
            test_paths=["tests/integration/"], num_runs=3, test_type="integration"
        )
        ...
        if flaky_tests:
            ...
            pytest.fail(
                "Flaky integration tests detected:\n"
                + "\n".join(flaky_list)
                + f"\n\nTotal flaky tests: {len(flaky_tests)}"
            )
        else:
    ...
    def test_detect_flaky_contract_tests(self):
        ...
        """
        flaky_tests = self._run_multiple_test_runs(
            test_paths=["tests/contract/"], num_runs=5, test_type="contract"
        )
    ...
    def _run_multiple_test_runs(
        self, test_paths: list[str], num_runs: int, test_type: str
    ) -> list[dict[str, Any]]:
        """
    ...
    def _run_single_test_run(
        self, test_paths: list[str], run_num: int
    ) -> list[dict[str, Any]]:
        """
        ...
        cmd = [
            ...
            "--json-report-file",
            f"/tmp/test-results-run-{run_num}.json",
        ] + test_paths
    ...
    def _parse_results_from_output(
        self, output: str, run_num: int
    ) -> list[dict[str, Any]]:
        """
