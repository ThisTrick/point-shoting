...
class TestEnginePipeline:
    ...
    def setup_method(self):
        ...
        self.test_image = (
            Path(__file__).parent.parent.parent / "examples" / "test_image.png"
        )
        ...
        try:
            while control.is_running() and (time.time() - start_time) < max_runtime:
                ...
                if frame_count % 30 == 0:
                    ...
                    if metrics:
                        print(
                            f"Frame {frame_count}: Stage={metrics.stage.name}, "
                        ...
                        if (
                            metrics.stage == Stage.FINAL_BREATHING
                            and metrics.recognition >= 0.8
                        ):
                            break
            ...
            assert Stage.FINAL_BREATHING in stages_reached, (
                f"Never reached FINAL_BREATHING stage. Reached: {reached_stage_names}"
            )

            # Validate final state
            assert final_metrics.stage == Stage.FINAL_BREATHING, (
                f"Final stage should be FINAL_BREATHING, got {final_metrics.stage}"
            )
    ...
    def test_stage_transitions_with_timeouts(self):
        ...
        try:
            while (
                control.is_running() and (time.time() - start_time) < 60
            ):  # 1 minute max
                engine.step()
                frame_count += 1
                ...
                if (
                    current_stage
                    and (time.time() - stage_start_times[current_stage]) > 30
                ):
                    control.stop()
                    pytest.fail(
                        f"Stage {current_stage.name} timed out after 30 seconds"
                    )
            ...
            assert current_stage == Stage.FINAL_BREATHING, (
                f"Should reach FINAL_BREATHING, got {current_stage}"
            )
    ...
    def test_recognition_convergence(self):
        ...
        try:
            while (
                control.is_running() and frame_count < 2000
            ...
            if len(recognition_scores) > 1:
                ...
                increasing_count = sum(
                    1
                    for i in range(1, len(recognition_scores))
            ...
            final_metrics = control.get_metrics()
            if final_metrics:
                assert final_metrics.recognition >= 0.7, (
                    f"Final recognition {final_metrics.recognition} too low"
                )
    ...
    def test_performance_under_load(self):
        ...
        try:
            ...
            assert avg_frame_time <= 1.0 / 30, (
                f"Frame time {avg_frame_time:.4f}s above 33ms target"
            )

            # Check frame time consistency (shouldn't have huge spikes)
            max_frame_time = max(frame_times)
            assert max_frame_time <= 0.1, (
                f"Max frame time {max_frame_time:.4f}s too high (possible freeze)"
            )
