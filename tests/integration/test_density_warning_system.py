...
class TestDensityWarningSystem:
    ...
    def test_low_density_no_warnings(self):
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if step_times:
                ...
                assert avg_step_time < 0.1, (
                    f"HIGH average step time for LOW density: {avg_step_time:.4f}s"
                )
                assert max_step_time < 0.2, (
                    f"HIGH max step time for LOW density: {max_step_time:.4f}s"
                )
    ...
    def test_medium_density_acceptable_performance(self):
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if step_times:
                ...
                assert avg_step_time < 0.5, (
                    f"Excessive step time for MEDIUM density: {avg_step_time:.4f}s"
                )

            # Should have more particles than LOW density
            if particle_counts:
                avg_particle_count = np.mean(particle_counts)
                assert avg_particle_count > 100, (
                    f"Unexpectedly low particle count for MEDIUM: {avg_particle_count}"
                )
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            for _ in range(10):  # Fewer steps for HIGH density
                ...
                if particles is not None:
                    ...
                    array_memory = (
                        particles.position.nbytes
                        + particles.velocity.nbytes
                        + particles.color_rgba.nbytes
                    )
            ...
            if particle_counts:
                avg_particle_count = np.mean(particle_counts)
                np.max(particle_counts)

                # Should have significantly more particles than MEDIUM
                assert avg_particle_count > 1000, (
                    f"LOW particle count for HIGH density: {avg_particle_count}"
                )

                # Should be consistent (not dropping particles due to performance issues)
                particle_variance = (
                    np.var(particle_counts) if len(particle_counts) > 1 else 0
                )
                assert particle_variance < avg_particle_count * 0.1, (
                    f"Unstable particle count: variance {particle_variance}"
                )
            ...
            if step_times:
                ...
                assert max_step_time < 2.0, (
                    f"Excessive step time for HIGH density: {max_step_time:.4f}s"
                )
    ...
    def test_large_image_density_scaling(self):
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if particles is not None:
                ...
                expected_min_particles = (
                    5000  # Reasonable minimum for large high-density
                )
                assert particle_count >= expected_min_particles, (
                    f"Too few particles for large image: {particle_count}"
                )

                # Should not be excessive (memory/performance concern)
                expected_max_particles = 50000  # Reasonable maximum
                assert particle_count <= expected_max_particles, (
                    f"Excessive particles for large image: {particle_count}"
                )
    ...
    def test_density_profile_memory_usage(self):
        ...
        for density_profile in [
            ...
            with patch("PIL.Image.open", return_value=mock_image):
                ...
                if particles is not None:
                    ...
                    total_bytes = (
                        position_bytes + velocity_bytes + color_bytes + target_bytes
                    )
        ...
        if (
            DensityProfile.LOW in density_memory_usage
            and DensityProfile.MEDIUM in density_memory_usage
        ):
            assert (
                density_memory_usage[DensityProfile.MEDIUM]
                >= density_memory_usage[DensityProfile.LOW]
            ), "MEDIUM density should use more memory than LOW"

        if (
            DensityProfile.MEDIUM in density_memory_usage
            and DensityProfile.HIGH in density_memory_usage
        ):
            assert (
                density_memory_usage[DensityProfile.HIGH]
                >= density_memory_usage[DensityProfile.MEDIUM]
        ...
        for density, memory_bytes in density_memory_usage.items():
            ...
            assert memory_mb < 100, (
                f"Excessive memory usage for {density}: {memory_mb:.1f}MB"
            )
            ...
            if step_times_early and step_times_later:
                ...
                if early_avg > 0:
                    ...
                    assert degradation_ratio < 3.0, (
                        f"Significant performance degradation: {degradation_ratio:.3f}x"
                    )
    ...
    def test_density_warning_thresholds(self):
        ...
        for density, result in performance_results.items():
            assert result["total_time"] < 5.0, (
                f"Excessive total time for {density}: {result['total_time']:.3f}s"
            )
