            ...
            for step in range(50):  # Reduced for faster test
                ...
                if snapshot is not None:
                    ...
                    assert np.all(positions >= 0.0), (
                        f"Step {step}: positions below 0.0 detected"
                    )
                    assert np.all(positions <= 1.0), (
                        f"Step {step}: positions above 1.0 detected"
                    )
    ...
    def test_boundary_clamping_behavior(self):
        ...
        with patch(
            ...
            if snapshot is not None:
                ...
                np.any(
                    (positions <= edge_tolerance)
                    | (positions >= (1.0 - edge_tolerance))
                )
        ...
        with patch(
            ...
            if initial_snapshot is not None:
                ...
                assert np.all(initial_positions >= 0.0), (
                    "Initial positions below 0.0 detected"
                )
                assert np.all(initial_positions <= 1.0), (
                    "Initial positions above 1.0 detected"
                )
            ...
            for step in range(10):  # Check first 10 steps of burst
                ...
                if snapshot is not None:
                    ...
                    assert np.all(positions >= 0.0), (
                        f"Burst step {step}: positions below 0.0"
                    )
                    assert np.all(positions <= 1.0), (
                        f"Burst step {step}: positions above 1.0"
                    )
