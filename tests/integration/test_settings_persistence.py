...
class TestSettingsPersistence:
    ...
    def test_settings_save_load_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if hasattr(original_settings, 'locale') and hasattr(loaded_settings, 'locale'):
                assert loaded_settings.locale == original_settings.locale, "Locale should match"
            if hasattr(original_settings, 'hud_enabled') and hasattr(loaded_settings, 'hud_enabled'):
                assert loaded_settings.hud_enabled == original_settings.hud_enabled, "HUD enabled should match"
    ...
    def test_settings_default_location(self):
        ...
        assert loaded_settings is not None, "Should load default settings when file missing"
    ...
    def test_settings_validation_on_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            loaded_settings = store.load(settings_path)
            assert loaded_settings is not None, "Should load settings even with invalid data"
            ...
            assert isinstance(loaded_settings, Settings), "Should return Settings instance"
