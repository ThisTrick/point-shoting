...
class TestAspectRatioPreservation:
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if particles is not None:
                ...
                if x_range > 0 and y_range > 0:
                    ...
                    assert 0.7 < aspect_ratio < 1.3, (
                        f"Square image aspect ratio distorted: {aspect_ratio:.3f}"
                    )
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if particles is not None:
                ...
                if x_extent > 0 and y_extent > 0:
                    ...
                    assert target_aspect_ratio > 1.5, (
                        f"Wide aspect ratio not preserved: {target_aspect_ratio:.3f}"
                    )
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if particles is not None:
                ...
                if x_extent > 0 and y_extent > 0:
                    ...
                    assert target_aspect_ratio < 0.8, (
                        f"Portrait aspect ratio not preserved: {target_aspect_ratio:.3f}"
                    )
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if particles is not None:
                ...
                if target_x_range > 0 and target_y_range > 0:
                    ...
                    assert extreme_aspect_ratio > 2.0, (
                        f"Extreme wide aspect not preserved: {extreme_aspect_ratio:.3f}"
                    )
                ...
                assert position_x_range > 0.1, (
                    f"Particles not distributed across image width: {position_x_range:.3f}"
                )
    ...
    def test_different_image_formats_consistent_aspect(self):
        ...
        if len(aspect_ratios_measured) > 1:
            ...
            assert aspect_variance < 0.1, (
                f"Inconsistent aspect ratios across formats: {aspect_ratios_measured}"
            )
    ...
    def test_particle_density_maintains_aspect_ratio(self):
        ...
        if len(aspect_ratios_by_density) > 1:
            ...
            assert ratio_variance < 0.05, (
                f"Aspect ratio varies with density: {aspect_ratios_by_density}"
            )

            # All should be approximately 2:1
            for ratio in ratios:
                assert 1.5 < ratio < 2.5, (
                    f"Aspect ratio not maintained across densities: {ratio:.3f}"
                )
    ...
    def test_aspect_ratio_during_animation_stages(self):
        ...
        with patch("PIL.Image.open", return_value=mock_image):
            ...
            if stage_aspect_ratios:
                ...
                assert np.all(all_ratios > 0.1), (
                    f"Aspect ratios too small: {np.min(all_ratios):.3f}"
                )
                assert np.all(all_ratios < 20.0), (
                    f"Aspect ratios too large: {np.max(all_ratios):.3f}"
                )
                ...
                if final_ratios:
                    ...
                    assert 0.5 < final_avg_ratio < 10.0, (
                        f"Final aspect ratio unreasonable: {final_avg_ratio:.3f}"
                    )
