...
class TestRecognitionMonotonic:
    ...
    def test_recognition_non_decreasing_in_formation(self):
        ...
        with patch('src.point_shoting.services.particle_engine.Image.open') as mock_open:
            ...
            for step, stage, recognition in all_recognition_scores:
                assert recognition >= 0.0, f"Step {step}: negative recognition score"
                assert recognition <= 1.1, f"Step {step}: recognition score too high (>1.1): {recognition}"
    ...
    def test_recognition_enforcement_mechanism(self):
        ...
        with patch('src.point_shoting.services.particle_engine.Image.open') as mock_open:
            ...
            for step in range(50):
                ...
                assert isinstance(recognition, (int, float)), f"Step {step}: recognition not numeric"
                assert not (recognition != recognition), f"Step {step}: recognition is NaN"  # NaN check
            ...
            assert min_recognition >= 0.0, f"Recognition went below 0: {min_recognition}"
            assert max_recognition <= 2.0, f"Recognition unreasonably high: {max_recognition}"  # Allow some slack
            
            # Test that recognition changes over time (not stuck at constant value)
            unique_values = len(set(recognition_values))
            assert unique_values > 1, "Recognition appears to be stuck at constant value"
