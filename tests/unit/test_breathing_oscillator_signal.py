...
class TestBreathingOscillatorSignal:
    ...
    def test_oscillation_amplitude_bounds(self):
        ...
        assert all(-0.2 <= v <= 0.2 for v in values), (
            "Oscillation exceeded amplitude bounds"
        )
    ...
    def test_phase_offset_behavior(self):
        ...
        oscillator2.configure(
            amplitude=0.1, frequency=frequency, phase_offset=math.pi / 2
        )
        ...
        assert abs(val1_t0) < 0.01, (
            f"Expected ~0 at t=0 for no phase offset, got {val1_t0}"
        )
        assert abs(val2_t0 - 0.1) < 0.01, (
            f"Expected ~0.1 at t=0 for π/2 phase offset, got {val2_t0}"
        )
        ...
        assert abs(val1_quarter - 0.1) < 0.01, (
            "Expected ~0.1 at T/4 for no phase offset"
        )
        assert abs(val2_quarter) < 0.01, "Expected ~0 at T/4 for π/2 phase offset"
