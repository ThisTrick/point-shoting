...
class TaskParser:
    ...
    def _parse_task_group(self, lines: list[str]) -> Task | None:
...
class MatrixGenerator:
    ...
    def generate_matrix(self, tasks: list[Task]) -> TraceabilityMatrix:
        ...
        uncovered_requirements = (self.KNOWN_FR - covered_fr) | (
            self.KNOWN_NFR - covered_nfr
        )
...
class MatrixRenderer:
    ...
    def render_markdown(self, matrix: TraceabilityMatrix) -> str:
        ...
        output.append(
            f"**Tasks with FR/NFR**: {len(matrix.tasks) - len(matrix.tasks_without_refs)}"
        )
        output.append("")
        ...
        output.append(
            f"- **Functional Requirements**: {fr_covered}/{fr_total} ({fr_covered / fr_total * 100:.1f}%)"
        )
        output.append(
            f"- **Non-Functional Requirements**: {nfr_covered}/{nfr_total} ({nfr_covered / nfr_total * 100:.1f}%)"
        )
        output.append("")
...
"""
        ...
        for fr in sorted(MatrixGenerator.KNOWN_FR):
            ...
            html += (
                f"        <tr><td>{fr}</td><td>{task_list}</td><td>{status}</td></tr>\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate traceability matrix from tasks.md"
    )
    parser.add_argument(
        "--output",
        "-o",
