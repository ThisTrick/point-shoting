...
def fix_file(filepath):
    ...
    new_content = re.sub(r'len\(engine\.particle_arrays\.positions\)', 'len(engine.get_particle_snapshot().positions)', content)
    if new_content != content:
        changed = True
        content = new_content
    
    # Handle cases like: engine.particle_arrays.positions
    new_content = re.sub(r'engine\.particle_arrays\.positions', 'engine.get_particle_snapshot().positions', content)
    if new_content != content:
        changed = True
        content = new_content
    
    # Handle cases like: engine.particle_arrays is not None
    new_content = re.sub(r'engine\.particle_arrays is not None', 'engine.get_particle_snapshot() is not None', content)
    if new_content != content:
        changed = True
        content = new_content
        
    # Handle cases like: assert engine.particle_arrays
    new_content = re.sub(r'assert engine\.particle_arrays', 'assert engine.get_particle_snapshot()', content)
    if new_content != content:
        changed = True
        content = new_content
    ...
    new_content = re.sub(r'engine\._stage = Stage\.FINAL_BREATHING', '# Stage mocking removed - use proper mocks', content)
    if new_content != content:
        changed = True
        content = new_content
