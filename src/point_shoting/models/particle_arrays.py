...
class ParticleArrays:
    ...
    def validate(self) -> None:
        ...
        # Check shapes
        assert self.position.shape == (N, 2), (
            f"position shape mismatch: {self.position.shape} != ({N}, 2)"
        )
        assert self.velocity.shape == (N, 2), (
            f"velocity shape mismatch: {self.velocity.shape} != ({N}, 2)"
        )
        assert self.target.shape == (N, 2), (
            f"target shape mismatch: {self.target.shape} != ({N}, 2)"
        )
        assert self.color_rgba.shape == (N, 4), (
            f"color_rgba shape mismatch: {self.color_rgba.shape} != ({N}, 4)"
        )
        assert self.active.shape == (N,), (
            f"active shape mismatch: {self.active.shape} != ({N},)"
        )
        assert self.stage_mask.shape == (N,), (
            f"stage_mask shape mismatch: {self.stage_mask.shape} != ({N},)"
        )

        # Check data types
        assert self.position.dtype == np.float32, (
            f"position dtype: {self.position.dtype} != float32"
        )
        assert self.velocity.dtype == np.float32, (
            f"velocity dtype: {self.velocity.dtype} != float32"
        )
        assert self.target.dtype == np.float32, (
            f"target dtype: {self.target.dtype} != float32"
        )
        assert self.color_rgba.dtype == np.uint8, (
            f"color_rgba dtype: {self.color_rgba.dtype} != uint8"
        )
        assert self.active.dtype == bool, f"active dtype: {self.active.dtype} != bool"
        assert self.stage_mask.dtype == np.uint8, (
            f"stage_mask dtype: {self.stage_mask.dtype} != uint8"
        )

        # Check bounds
        assert np.all(self.position >= 0.0) and np.all(self.position <= 1.0), (
            "positions out of [0,1] bounds"
        )
        assert np.all(self.target >= 0.0) and np.all(self.target <= 1.0), (
            "targets out of [0,1] bounds"
        )
...
def allocate_particle_arrays(particle_count: int) -> ParticleArrays:
    ...
    color_rgba = np.full(
        (particle_count, 4), 255, dtype=np.uint8
...
def map_image_to_targets(
    arrays: ParticleArrays, image_array: np.ndarray, preserve_aspect: bool = True
) -> None:
    """
    ...
    if image_array.size == 0 or len(image_array.shape) < 2:
        raise ValueError(
            f"Invalid image array: shape={image_array.shape}, size={image_array.size}"
        )
...
def initialize_burst_positions(
    ...
    burst_points: np.ndarray | None = None,
    ...
    if burst_points.shape[1] != 2:
        raise ValueError(
            f"burst_points must have shape (n_bursts, 2), got {burst_points.shape}"
        )
    ...
    burst_centers = burst_points[
        burst_assignments
    ]  # Should be shape: (N, 2)    # Add offsets vectorized
