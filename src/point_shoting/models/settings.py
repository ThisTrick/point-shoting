...
class Settings:
    ...
    def validate(self) -> None:
        ...
        if not (0.0 <= self.breathing_amplitude <= 0.03):
            raise ValueError(
                f"breathing_amplitude must be in [0.0, 0.03], got {self.breathing_amplitude}"
            )
        ...
        if self.chaos_min_duration < 0:
            raise ValueError(
                f"chaos_min_duration must be ≥0, got {self.chaos_min_duration}"
            )

        if self.formation_fallback_time <= 0:
            raise ValueError(
                f"formation_fallback_time must be >0, got {self.formation_fallback_time}"
            )

        if self.stable_frames_threshold < 1:
            raise ValueError(
                f"stable_frames_threshold must be ≥1, got {self.stable_frames_threshold}"
            )

        if self.recognition_computation_interval < 1:
            raise ValueError(
                f"recognition_computation_interval must be ≥1, got {self.recognition_computation_interval}"
            )
