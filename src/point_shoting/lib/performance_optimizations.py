...
def vectorized_burst_physics(particles, dt: float, stage_progress: float, 
                           physics_params) -> None:
    """
...
def vectorized_chaos_physics(particles, dt: float, physics_params) -> None:
    ...
    random_forces = np.random.uniform(-1, 1, particles.velocity.shape) * physics_params.noise_strength
    ...
    target_distances = np.linalg.norm(target_dirs, axis=1, keepdims=True)  # Shape: (N, 1)
...
def vectorized_breathing_physics(particles, dt: float, physics_params, 
                               breathing_oscillator, stage_elapsed: float) -> None:
    """
...
def batch_color_update(particles, color_mapper, target_image_data: Optional[np.ndarray] = None) -> None:
    """
