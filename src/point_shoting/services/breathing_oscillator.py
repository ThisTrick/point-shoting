...
class BreathingOscillator:
    ...
    def get_oscillation(self, time_sec: float) -> float:
        ...
        phase = (
            2 * math.pi * self._params.frequency * effective_time
    ...
    def get_batch_oscillation(self, time_sec: float, count: int) -> np.ndarray:
        ...
        phases = (
            2 * np.pi * self._params.frequency * effective_times
            + self._params.phase_offset
        )
    ...
    def get_radial_breathing(
        self, time_sec: float, center: np.ndarray, positions: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def get_alpha_breathing(self, time_sec: float, base_alpha: float = 1.0) -> float:
        ...
        alpha_modulation = (
            0.5 + 0.5 * oscillation
        )  # Maps [-amp, +amp] to [0.5-amp/2, 0.5+amp/2]
    ...
    def auto_adjust_amplitude(
        self, target_rms: float = 0.05, adjustment_rate: float = 0.1
    ) -> None:
        """
    ...
    def create_harmonic_layer(
        self, base_frequency: float, harmonic: int = 2
    ) -> "BreathingOscillator":
        """
        ...
        harmonic_osc.configure(
            ...
            amplitude=self._params.amplitude
            / harmonic,  # Reduce amplitude for higher harmonics
            phase_offset=self._params.phase_offset,
            decay=self._params.decay,
        )
        return harmonic_osc
