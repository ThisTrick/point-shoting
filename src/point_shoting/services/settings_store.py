...
class SettingsStore:
    ...
    def load(self, file_path: Path | None = None) -> Settings:
        """
        ...
        try:
            if not file_path.exists():
                self.logger.info(
                    f"Settings file not found: {file_path}, using defaults"
                )
            ...
            if not isinstance(data, dict):
                self.logger.warning(
                    f"Settings file contains invalid data type: {type(data)}, using defaults"
                )
                return Settings.default()
            ...
            if removed_keys:
                self.logger.warning(
                    f"Filtered out unknown setting keys: {removed_keys}"
                )
            ...
            self.logger.error(
                f"Settings file corrupted (invalid JSON): {file_path} - {e}"
            )
            return Settings.default()

        except (ValueError, TypeError) as e:
            self.logger.error(
                f"Settings file contains invalid values: {file_path} - {e}"
            )
            return Settings.default()
    ...
    def save(self, settings: Settings, file_path: Path | None = None) -> bool:
        """
    ...
    def load_or_create_default(self, file_path: Path | None = None) -> Settings:
        """
    ...
    def backup_settings(self, file_path: Path | None = None) -> Path | None:
        """
    ...
    def clear_settings(self, file_path: Path | None = None) -> bool:
        """
