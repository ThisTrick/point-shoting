...
class WatermarkRenderer:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._watermark_image: Image.Image | None = None
        self._text_watermark: str | None = None
        self._font_path: str | None = None
        self._font_size: int = 24
        self._validation_errors: list[str] = []

    def load_png_watermark(self, png_path: str | Path) -> bool:
        """
        ...
        if not PIL_AVAILABLE:
            self._validation_errors.append(
                "PIL/Pillow not available for PNG processing"
            )
            return False
        ...
        try:
            ...
            if image.format != "PNG":
                self._validation_errors.append(
                    f"File format is {image.format}, expected PNG"
                )
                return False
    ...
    def set_text_watermark(
        self, text: str, font_path: str | None = None, font_size: int = 24
    ) -> bool:
        """
    ...
    def _calculate_position(
        self, target_size: tuple[int, int], watermark_size: tuple[int, int]
    ) -> tuple[int, int]:
        ...
        if self._config.position == WatermarkPosition.TOP_LEFT:
            ...
            return (
                target_width - watermark_width - margin,
                target_height - watermark_height - margin,
            )
    ...
    def validate_png(self, png_path: str | Path) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if png_path.suffix.lower() != ".png":
                validation_result["warnings"].append(
                    f"File extension is {png_path.suffix}, expected .png"
                )
            ...
            with Image.open(png_path) as img:
                ...
                if img.format != "PNG":
                    validation_result["errors"].append(
                        f"Format is {img.format}, expected PNG"
                    )
                    return validation_result
                ...
                if img.mode not in ("RGBA", "LA", "P"):
                    validation_result["warnings"].append(
                        f"Mode {img.mode} may not support transparency"
                    )
