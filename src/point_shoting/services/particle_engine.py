...
from ..lib.performance_optimizations import (
    optimized_chaos_energy,
    optimized_recognition_score,
    vectorized_breathing_physics,
    vectorized_burst_physics,
    vectorized_chaos_physics,
    vectorized_converging_physics,
    vectorized_formation_physics,
)
from ..models.metrics import Metrics
from ..models.particle_arrays import (
    ParticleArrays,
...
class ParticleEngine:
    ...
    def __init__(self) -> None:
        ...
        self._particles: ParticleArrays | None = None
        self._stage_state = StageState()
        self._physics_params = PhysicsParams()

        # Services - will be initialized in init()
        self._transition_policy: StageTransitionPolicy | None = None
        self._color_mapper: ColorMapper | None = None
        self._breathing_oscillator: BreathingOscillator | None = None
    ...
    def init(self, settings: Settings, image_path: str) -> None:
        ...
        try:
            ...
            if hasattr(self._target_image, "size") and hasattr(
                self._target_image.size, "__iter__"
            ):
                try:
                    ...
                    if min_dimension < 32:  # Minimum size threshold
                        ...
                        new_size = (
                            self._target_image.size[0] * scale_factor,
        ...
        if image_array.size == 0 or len(image_array.shape) < 2:
            ...
            if hasattr(self._target_image, "size") and hasattr(
                ...
                width, height = 100, 100
            image_array = (
                np.zeros((height, width, 3), dtype=np.uint8) + 128
            )  # Gray image
    ...
    def step(self) -> None:
        ...
        dt = (
            current_time - self._last_step_time
            if self._last_step_time > 0
            else self._physics_params.dt
        )
    ...
    def _update_stage_timing(self, current_time: float) -> None:
        ...
        self._stage_state.stage_elapsed = (
            current_time - self._stage_state.stage_start_time
        )

        # Calculate stage progress (rough approximation)
        expected_duration = self._get_expected_stage_duration(
            self._stage_state.current_stage
        )
        if expected_duration > 0:
            self._stage_state.stage_progress = min(
                1.0, self._stage_state.stage_elapsed / expected_duration
            )
        else:
    ...
    def _update_particle_colors(self) -> None:
        ...
        colors = self._color_mapper.batch_color_assignment(
            self._particles.target, self._settings.color_mode
        )
    ...
    def get_metrics(self) -> Metrics:
        ...
        frame_time_ms = (
            (sum(self._step_times) / len(self._step_times)) * 1000
            if self._step_times
            else 0.0
        )
    ...
    def get_particle_snapshot(self) -> ParticleArrays | None:
    ...
    def _calculate_recognition_score(self) -> float:
        ...
        return optimized_recognition_score(
            self._particles.position, self._particles.target
        )
